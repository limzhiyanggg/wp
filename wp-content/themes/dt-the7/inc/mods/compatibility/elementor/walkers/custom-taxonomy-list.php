...
class Custom_Taxonomy_List extends \Walker {
	...
	public function start_el( &$output, $cat, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( $depth < 1 ) {
			$output .= '"><a href="' . get_term_link( $cat_id, $this->tree_type ) . '" ><span class="item-content">' . $this->get_term_name( $cat ) . '</span>' . $sub_cat_count . '<span class="next-level-button"> ' . $sub_cat_icon . $sub_cat_act_icon . '</span></a>';
		} else {
			$output .= '"><a href="' . get_term_link( $cat_id, $this->tree_type ) . '" ><span class="item-content">' . $this->get_term_name( $cat ) . '</span>' . $sub_cat_count . '<span class="next-level-button"> ' . $sub_sub_cat_icon . $sub_sub_cat_act_icon . '</span></a>';
		}
	}
